...
/***/ "../extension-polkagate/src/util/workers/getMyPools.js":
/*!*************************************************************!*\
  !*** ../extension-polkagate/src/util/workers/getMyPools.js ***!
  \*************************************************************/
...
/* harmony import */ var _getApi__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../getApi */ "../extension-polkagate/src/util/getApi.ts");
...
async function getMyPools(address, endpoint) {
  ...
  const api = await (0,_getApi__WEBPACK_IMPORTED_MODULE_0__["default"])(endpoint);
  const lastPoolId = await api.query.nominationPools.lastPoolId();
  console.log(`getMyPools: Getting ${lastPoolId.toNumber()} pools information.`);
  ...
  for (let poolId = 1; poolId <= lastPoolId.toNumber(); poolId++) {
    queries.push(Promise.all([api.query.nominationPools.metadata(poolId), api.query.nominationPools.bondedPools(poolId), api.query.nominationPools.rewardPools(poolId)]));
  }

  const info = await Promise.all(queries);
  const poolsInfo = (_info$map = info.map((i, index) => {
    if (i[1].isSome) {
      var _i$, _i$2, _i$3, _i$4, _i$5;
      ...
      return {
        ...
        metadata: (_i$ = i[0]) != null && _i$.length ? (_i$2 = i[0]) != null && _i$2.isUtf8 ? (_i$3 = i[0]) == null ? void 0 : _i$3.toUtf8() : (_i$4 = i[0]) == null ? void 0 : _i$4.toString() : null,
        poolId: index + 1,
        // works because pools id is not reuseable for now
        rewardPool: (_i$5 = i[2]) != null && _i$5.isSome ? i[2].unwrap() : null
      };
    } else {
      return undefined;
    }
  })) == null ? void 0 : _info$map.filter(f => f !== undefined);
  const filteredPools = poolsInfo.filter(poolInfo => {
    var _poolInfo$bondedPool, _poolInfo$bondedPool2, _poolInfo$bondedPool3;

    return String(poolInfo == null ? void 0 : (_poolInfo$bondedPool = poolInfo.bondedPool) == null ? void 0 : _poolInfo$bondedPool.roles.root) === address || String(poolInfo == null ? void 0 : (_poolInfo$bondedPool2 = poolInfo.bondedPool) == null ? void 0 : _poolInfo$bondedPool2.roles.stateToggler) === address || String(poolInfo == null ? void 0 : (_poolInfo$bondedPool3 = poolInfo.bondedPool) == null ? void 0 : _poolInfo$bondedPool3.roles.nominator) === address;
  });
  const token = api.registry.chainTokens[0];
  const decimal = api.registry.chainDecimals[0];
  const myPools = filteredPools.map(async poolInfo => {
    var _poolInfo$metadata;

    const accounts = (0,_getPoolAccounts__WEBPACK_IMPORTED_MODULE_1__["default"])(api, poolInfo.poolId);
    ...
    const [rewardPools, rewardIdBalance, stashIdAccount] = await Promise.all([api.query.nominationPools.rewardPools(poolInfo.poolId), api.query.system.account(accounts.rewardId), api.derive.staking.account(accounts.stashId)]);
    ...
    const pool = {
      ...
      decimal,
      member: undefined,
      metadata: (_poolInfo$metadata = poolInfo.metadata) != null && _poolInfo$metadata.length ? poolInfo.metadata.isUtf8 ? poolInfo.metadata.toUtf8() : poolInfo.metadata.toString() : null,
...
/******/ 		var __webpack_exports__ = __webpack_require__.O(undefined, ["vendors-node_modules_polkadot_api_promise_Api_js"], () => (__webpack_require__("../extension-polkagate/src/util/workers/getMyPools.js")))
...
/******/ 		var installedChunks = {
/******/ 			"extension-polkagate_src_util_workers_getMyPools_js": 1
/******/ 		};
/******/ 		
